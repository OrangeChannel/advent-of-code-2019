...
def intcode(input_val, ilist: list, k=0):
    ...
    while True:
        ...
        if code == 99: break
            ...
            k = get(decipher(s)['modes'][1], ilist[k + 2]) if \
                get(decipher(s)['modes'][0], ilist[k + 1]) else k + 3
        elif code == 6:  # jump if False
            k = get(decipher(s)['modes'][1], ilist[k + 2]) if not \
                get(decipher(s)['modes'][0], ilist[k + 1]) else k + 3
        elif code == 7:  # less than
            ilist[ilist[k + 3]] = 1 if \
                get(decipher(s)['modes'][0], ilist[k + 1]) < \
                get(decipher(s)['modes'][1], ilist[k + 2]) else 0
        elif code == 8:  # equals
            ilist[ilist[k + 3]] = 1 if \
                get(decipher(s)['modes'][0], ilist[k + 1]) == \
                get(decipher(s)['modes'][1], ilist[k + 2]) else 0
